    ...
    <>
      <section className="coment-box conteiner">
        <div className="comentarios">
          {loading ? (
            <div className="w-full h-20 bg-zinc-300 animate-pulse rounded-md"></div>
          ) : (
            <form onSubmit={sendComment}>
              <textarea
                name="texto"
                className="p-5 border-2 border-zinc-400 focus:border-zinc-600 outline-none rounded-md transition-colors duration-300"
                value={commentText}
          ...
          {comentarios.length > 0 ? (
            comentarios.map((comentario, index) => (
              <div
                key={index}
                className="comentario-single"
                style={{ marginBottom: "50px" }}
              >
                <div className="img-coment">
                  <img
                    src={
                      comentario.userId.avatar == "e"
                        ? userImg
                        : comentario.userId.avatar
                    }
                    alt="Foto de perfil"
                  />
                </div>
                <div className="coment-user">
                  ...
                  >
                    ...
                    >
                      {howMuchPass(comentario.createdAt)}
                    </p>
                  </div>
                  <p
                    style={{
                  ...
                  >
                    <div style={{ display: "flex", gap: "10px" }}>
                      <FontAwesomeIcon
                        ...
                        onClick={() => handleLike(comentario._id)}
                      />
                    ...
                    <div style={{ display: "flex", gap: "10px" }}>
                      <FontAwesomeIcon
                        ...
                        icon={faThumbsDown}
                        style={{ cursor: "pointer" }}
                        onClick={() => handleDislike(comentario._id)}
                      />
          ...
          {mediaEstrelas ? (
            <>
              <h3 className="text-lg font-bold">
                {isNaN(mediaEstrelas.media)
                  ? "Sem avaliações"
                  : mediaEstrelas.media.toString().slice(0, 3)}
              </h3>
              <p>Nota média</p>
            </>
          ) : (
            <div className="w-20 h-16 m-auto bg-zinc-300 animate-pulse rounded-md"></div>
          )}
          {loading ? (
            <div className="w-80 m-auto mt-4 h-6 bg-zinc-300 animate-pulse rounded-md"></div>
          ) : (
            <div className="qtd-stars">
              ...
              >
                <div
                  ...
                  style={{
                    width:
                      mediaEstrelas?.qtdEstrelas &&
                      `${
                        mediaEstrelas?.qtdEstrelas.find(
                          (item) => Object.keys(item)[0] == "cinco"
                        )?.cinco
                      }%`,
                ...
                <div
                  ...
                  style={{
                    width:
                      ...
                      `${
                        mediaEstrelas.qtdEstrelas.find(
                          (item) => Object.keys(item)[0] == "quatro"
                        )?.quatro
                      }%`,
                    backgroundColor: "yellow",
                    height: "10px",
